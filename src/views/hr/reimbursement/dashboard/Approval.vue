...
// get plafon data
onMounted(async () => {
    const result = await axiosHR.get('/reimbursment/transactions/approval',{
                params:{
...
const saveProduct = async (status_value) => {
    product.value.token = localStorage.getItem('token');
    product.value.user_local = localStorage.getItem('id');
    product.value.status = status_value;
    const data = product.value;
    const result = await axiosHR.post('/reimbursment/transactions/approval',data);
    if(result.status == 200){
        product.value = result.data.metadata.data;
        products.value[findIndexById(product.value.id)] = product.value;
        if(product.value.status_id == 2){ // approved
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Data approved', life: 9000 });
            acceptProductDialog.value = false;
        } 
        if(product.value.status_id == 3){ // rejected
            toast.add({ severity: 'error', summary: 'Warning', detail: 'Data decline', life: 9000 });
        ...
        <div class="col-12">
            <div class="card">
                ...
                >
                    ...
                    <Column field="transaction_id" header="Transaction Id" :sortable="true" headerStyle="width:20%; min-width:10rem;">
                    ...
                    <Column field="status" header="Status" :sortable="true" headerStyle="width:20%; min-width:10rem;">
                        <template #body="slotProps">
                            <Tag class="mr-2" :severity="slotProps.data.status_severity" :value="slotProps.data.status_label" :rounded="true"></Tag>
                        </template>
                    </Column>
                    <Column field="action" header="Action" headerStyle="width:20%; min-width:10rem;">
                        <template #body="slotProps" >
                            <div v-if="slotProps.data.status_id == 0" >
                                <Button icon="pi pi-check" class="p-button-rounded p-button-success mr-2" @click="confirmAcceptProduct(slotProps.data)" />
                ...
                <Dialog v-model:visible="acceptProductDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
                    ...
                    <template #footer>
                        ...
                        <Button label="Yes" icon="pi pi-check" severity="success" text @click="saveProduct(true)" />
                    </template>
                </Dialog>
                ...
                <Dialog v-model:visible="deleteProductDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
                    <div class="flex align-items-center justify-content-center">
                        <i class="pi pi-exclamation-triangle mr-3" style="font-size: 2rem" />
                        <span v-if="product"
                            >Are you sure you want to decline <b>{{ product.request_id }}</b
                            >?</span
                        >
                    </div>
                    <template #footer>
                        <Button label="Cancel" icon="pi pi-times" class="p-button-text" @click="deleteProductDialog = false" />
                        <Button label="Yes" icon="pi pi-check" severity="danger" text @click="saveProduct(false)" />
                    </template>
                </Dialog>
                <!-- status in row end -->
